...
use crate::internal::measurement::Measurement;
    ...
    D: DelayNs,
{
    ...
    async fn read_command(&mut self, cmd: Command, buf: &mut [u8]) -> Result<(), Error<E>> {
        ...
        self.delay.delay_ms(READ_DELAY_MS).await;
        self.i2c
    ...
    /// before read out.
    pub async fn measurement(&mut self) -> Result<Measurement, Error<E>> {
        let mut buf = [0; 18];
    ...
    pub async fn read_firmware_version(&mut self) -> Result<(u8, u8), Error<E>> {
        let mut buf = [0; 3];
        self.read_command(READ_FIRMWARE_VERSION, &mut buf).await?;
